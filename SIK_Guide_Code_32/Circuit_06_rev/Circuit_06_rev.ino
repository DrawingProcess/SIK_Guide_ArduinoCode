...
void loop()
{
  ...
  // map(value(매핑하고자 하는 값(현재범위)), fromLow(현재 범위의 하한값), fromHigh(현재 범위의 상한값), 
  //     toLow(매핑하고자 하는 범위의 하한값), toHigh(매핑하고자 하는 범위의 상한값)) -> 범위변경(return: 매핑된 값)
  // constrain(value, min, max) ->value의 값이 min과 max 사이라면 value, min이하(max이상)라면 min(max)
  ...
  reverseTune();  // reverse autoTune(); 255 - lightLevel
...
void reverseTune()
{
  ...
  lightLevel = map(lightLevel, low+30, high-30, 0, 255);
  lightLevel = constrain(255 - lightLevel, 0, 255);
